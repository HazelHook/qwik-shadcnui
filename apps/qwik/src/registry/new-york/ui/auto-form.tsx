...
function AutoFormInput({ label, isRequired, fieldConfigItem, fieldProps }: AutoFormInputComponentProps) {
	return (
		<FormItem>
			<FormLabel>
				{label}
				{isRequired && <span class="text-destructive"> *</span>}
			</FormLabel>
			<FormControl>
			...
			<FormControl>
				...
				>
					{values.map((value: any) => (
						<FormItem class="flex items-center space-x-3 space-y-0" key={value}>
							<FormControl>
		...
		<FormItem>
			...
			<FormControl>
				<Select onValueChange={field.onChange} defaultValue={field.value}>
					<SelectTrigger>
						<SelectValue class="w-full" placeholder={fieldConfigItem.inputProps?.placeholder}>
...
function AutoFormObject<SchemaType extends z.ZodObject<any, any>>({
	...
	return (
		<Accordion type="multiple" class="space-y-5">
...
function AutoFormArray({
	...
	return (
		<AccordionItem value={name}>
			...
			<AccordionContent class="border-l p-3 pl-6">
				{fields.map((_field, index) => {
					const key = [...path, index.toString()].join(".")
					return (
						<div class="mb-4 grid gap-6" key={`${key}`}>
							<AutoFormObject
							...
							<Button variant="secondary" size="none" type="button" onClick={() => remove(index)}>
								<CopperXIcon class="h-4 w-4" />
							</Button>
							<Separator />
						</div>
					)
				})}
				<Button type="button" onClick={() => append({})} class="flex items-center">
					<AddIcon class="mr-2" size={16} />
					Add
				</Button>
			</AccordionContent>
		</AccordionItem>
