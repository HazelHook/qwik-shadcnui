...
const DialogTitle = component$(({ class: classes, ...props }: QwikIntrinsicElements["h2"]) => (
	<h2 q:slot="title" class={cn("text-lg font-semibold leading-none tracking-tight", classes?.toString())} {...props}>
		<Slot />
	</h2>
))

const DialogDescription = component$(({ class: classes, ...props }: QwikIntrinsicElements["h2"]) => (
	<p q:slot="description" class={cn("text-sm text-muted-foreground", classes?.toString())} {...props}>
		<Slot />
	</p>
))
